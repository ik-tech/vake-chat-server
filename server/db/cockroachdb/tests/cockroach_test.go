...
import (
	...
	"github.com/tinode/chat/server/db/cockroachdb"
	//backend "github.com/tinode/chat/server/db/cockroachdb"
...
func TestCredGetActive(t *testing.T) {
	...
	// Test not found
	cred, _ := adp.CredGetActive(types.ParseUserId("dummyusrid"), "")
	if cred != nil {
...
// IK-MARK Failed on seqid try it. seqid updated by TopicUpdateOnMessage. change test sequence
...
func initConnectionToDb() {
	if a, ok := adp.(cockroachdb.RawDB); ok {
		db = a.DB()
		return
	}
	log.Fatal("get raw DB")
}

func init() {
	logs.Init(os.Stdout, "stdFlags")
	conffile := flag.String("config", "./test.conf", "config of the database connection")

	if createDB {
		adp = cockroachdb.GetAdapter()
		...
		if err := store.Open(11, cfg); err != nil {
			log.Fatal("Store open")
		}

		adp = store.GetAdapter()
		if adp == nil {
